...
public class Foam
{
    ...
    public static Bitmap GenerateBubbleHeightMap(int width, int height, int numberOfBubbles)
    {
        ...
        for (var i = 0; i < numberOfBubbles; i++) points.Add(new PointF((float) (_random.NextDouble() * width), (float) (_random.NextDouble() * height)));
        ...
        for (var x = 0; x < width; x++)
        {
            ...
            {
                var dist = Distance(point, new PointF(x, y));
                if (dist < minDist)
                {
            ...
            heightMap.SetPixel(x, y, Color.FromArgb(colorValue, colorValue, colorValue));
        }
