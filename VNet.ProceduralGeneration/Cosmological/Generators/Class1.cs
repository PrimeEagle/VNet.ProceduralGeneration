...
public class SCircle
{
    ...
    public void UpdateBoundingBox()
    {
        ...
        var rotationAxis = Vector3.Cross(defaultOrientation, OrientationNormal);

        rotationAxis = rotationAxis.LengthSquared() < 1e-10 ? defaultOrientation : Vector3.Normalize(rotationAxis);
        var dot = Vector3.Dot(defaultOrientation, OrientationNormal);
        ...
        OrientedBoundingBox = new BoundingBox<float>(Center, Diameter / 2, Vector3.Transform(new Vector3(Diameter, Diameter, Thickness), rotationMatrix));
    }
...
public class FCylinder
{
    ...
    public Vector3 Direction
    {
        ...
        set
        {
            ...
            OrientedBoundingBox.Rotation = Matrix4x4.CreateFromQuaternion(rotation);
    ...
    public void UpdateBoundingBox()
    {
        ...
        var rotation = Quaternion.CreateFromAxisAngle(rotationAxis, rotationAngle);
        var rotationMatrix = Matrix4x4.CreateFromQuaternion(rotation);
        OrientedBoundingBox = new BoundingBox<float>(Center, Diameter / 2, Vector3.Transform(new Vector3(Diameter, Diameter, Length), rotationMatrix));
    }
...
public class CylinderMerger
{
    ...
    public List<FCylinder> MergeCylinders(List<FCylinder> cylinders)
    {
        ...
        while (anyMerged && iterations < MaxCylinderMergeIterations)
        {
            ...
            for (var i = 0; i < cylinders.Count; i++)
            {
                ...
                if (!merged) newCylinders.Add(cylinders[i]);
            }
    ...
    private bool CanMerge(FCylinder cyl1, FCylinder cyl2)
    {
        ...
        if (distance > MaxCylinderMergeDistance) return false;
